...
const uint16_t cCrc::INIT_CRC = 0xFFFF;
const uint16_t cCrc::GOOD_CRC = 0xF0B8;

const uint16_t cCrc::cCrcTable[256] =
{
...
uint16_t cCrc::ccitt_crc16(uint8_t data,uint16_t crc)
{
    return ((crc >> 8) ^ cCrcTable[(crc ^ data) & 0xff]);
}

uint16_t cCrc::ccitt_crc16(uint8_t * data_ptr,uint32_t len)
{
