...
cHDLCframer::cHDLCframer(uint32_t buff_len)
{
...
void cHDLCframer::frame(uint8_t * payload_ptr,
...
int cHDLCframer::pack(uint8_t byte)
{
	...
	else //middle or last bytes
	{
		if(byte == 0x7E)// && var->frameLen > 2)  //frame end
		{
			...
			uint16_t calc_crc = cCrc::ccitt_crc16(mBuffer,mFrameIdx);
...
// --------------------------------------------------
void cHDLCframer::add_byte(uint8_t byte, uint8_t  * frame_ptr, uint32_t * frame_length)
{
